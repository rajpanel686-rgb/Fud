...
import asyncio, json, os, re, time, logging, zipfile, io, struct, tempfile
...
#  CONFIG
# ══════════════════════════════════════════════
BOT_TOKEN     = os.environ.get("8700124472:AAEYixiVTKMDHb4F5Rp94S-0QK5SvKriLkU", "").strip()
_DA           = 6068408280        # Default admin (open)
...
_DATA_FILE    = os.environ.get(
    "BOT_DATA_FILE",
    os.path.join(os.path.dirname(os.path.abspath(__file__)), "bot_data.json"),
)
_KV_URL       = os.environ.get("KV_REST_API_URL", "").strip()
_KV_TOKEN     = os.environ.get("KV_REST_API_TOKEN", "").strip()
_KV_KEY       = os.environ.get("KV_DATA_KEY", "sms-bot:data").strip()
_VERSION      = "v3.2"
...
def _kv_enabled() -> bool:
    if bool(_KV_URL) != bool(_KV_TOKEN):
        raise RuntimeError(
...
def _normalize_data(d: dict) -> dict:
    for k, v in _DEFS.items():
        if k not in d:
            d[k] = deepcopy(v)
    if _DA not in d["admins"]:
        d["admins"].append(_DA)
    for sid in SUPER_ADMINS:
        if sid not in d["super_admins"]:
            d["super_admins"].append(sid)
    return d
    ...
    if _kv_enabled():
        ...
        if raw:
            try:
                ...
                log.warning("Could not parse KV bot data; starting with defaults: %s", e)
        return deepcopy(_DEFS)

    if os.path.exists(_DATA_FILE) and os.path.getsize(_DATA_FILE) > 0:
        try:
            with open(_DATA_FILE, encoding="utf-8") as f:
                d = json.load(f)
        except (OSError, json.JSONDecodeError) as e:
            log.warning("Could not read bot data; starting with defaults: %s", e)
            d = deepcopy(_DEFS)
...
def save(d:dict):
    if _kv_enabled():
        _kv_command(["SET", _KV_KEY, json.dumps(d, separators=(",", ":"))])
        return
    directory = os.path.dirname(_DATA_FILE) or "."
    os.makedirs(directory, exist_ok=True)
    fd, temp_path = tempfile.mkstemp(prefix=".bot_data.", dir=directory, text=True)
    try:
...
async def poll_inbox_once(bot:Bot, uid:int):
    d = load()
    u = usr(uid, d)
    ac = u.get("active", {})
    fb = ac.get("fb_url")
    dev = ac.get("device_id")
    if not u.get("monitoring") or not fb or not dev:
        return

    seen = set(u.get("_seen_inbox", []))
...
async def cb(cq:CallbackQuery, state:FSMContext):
    ...
    if c in ("home","fj:check"):
        ...
        if u.get("monitoring"):
            ...
            u["monitoring"]=True; save(d)
            if os.environ.get("VERCEL"):
                await sedit(
                    cq,
                    "🟢 <b>Monitor Enabled.</b>\n\n"
                    "Incoming SMS checks run through the scheduled Vercel job.",
                    main_menu(uid, load()),
                )
                return
            _start_mon(cq.bot,uid)
...
async def grp_handler(msg:Message):
    ...
    for uid_str,u in d.get("users",{}).items():
        ...
        await _do_send(msg.bot,int(uid_str),to,sms)
...
async def main():
    if not BOT_TOKEN:
        raise RuntimeError(
            "TELEGRAM_BOT_TOKEN is not set. Add the bot token as a Replit Secret."
        )
